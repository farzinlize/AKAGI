...
def make_pattern_report(name, pattern:ChainNode, sequences, directory=''):
    ...
    A = list(aligned_matrix['A'][start:end+1]);Al = list(aligned_matrix['A'][log_start:log_end+1])
    C = list(aligned_matrix['C'][start:end+1]);Cl = list(aligned_matrix['C'][log_start:log_end+1])
    T = list(aligned_matrix['T'][start:end+1]);Tl = list(aligned_matrix['T'][log_start:log_end+1])
    G = list(aligned_matrix['G'][start:end+1]);Gl = list(aligned_matrix['G'][log_start:log_end+1])
    ...
    rA = list(reversed(list(aligned_matrix['T'][r_start:r_end+1])));rAl = list(reversed(list(aligned_matrix['T'][r_log_start:r_log_end+1])))
    rC = list(reversed(list(aligned_matrix['G'][r_start:r_end+1])));rCl = list(reversed(list(aligned_matrix['G'][r_log_start:r_log_end+1])))
    rG = list(reversed(list(aligned_matrix['C'][r_start:r_end+1])));rGl = list(reversed(list(aligned_matrix['C'][r_log_start:r_log_end+1])))
    rT = list(reversed(list(aligned_matrix['A'][r_start:r_end+1])));rTl = list(reversed(list(aligned_matrix['A'][r_log_start:r_log_end+1])))

    data = {'A':[cal(A, i) for i in range(len(A))], 
            'T':[cal(T, i) for i in range(len(T))], 
            'C':[cal(C, i) for i in range(len(C))], 
            'G':[cal(G, i) for i in range(len(G))]}
    
    rdata = {'A':[cal(rA, i) for i in range(len(rA))], 
             'T':[cal(rT, i) for i in range(len(rT))], 
             'C':[cal(rC, i) for i in range(len(rC))], 
             'G':[cal(rG, i) for i in range(len(rG))]}

    datal = {'A':[cal(Al, i) for i in range(len(Al))], 
             'T':[cal(Tl, i) for i in range(len(Tl))], 
             'C':[cal(Cl, i) for i in range(len(Cl))], 
             'G':[cal(Gl, i) for i in range(len(Gl))]}
    
    rdatal = {'A':[cal(rAl, i) for i in range(len(rAl))], 
              'T':[cal(rTl, i) for i in range(len(rTl))], 
              'C':[cal(rCl, i) for i in range(len(rCl))], 
              'G':[cal(rGl, i) for i in range(len(rGl))]}
